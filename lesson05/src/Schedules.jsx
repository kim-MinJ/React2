...
export default function Schedules() {
  ...
  //삭제할 때 updatedSchedule 코드 보다 여러 속성값을 복사하는 경우가 많기 때문에
  ...
  // 특정 날짜, 시간 todo 항목 삭제 후 상태값 변수 schedules, slectedSchedule 변경
        ...
        {schedules.map((sch, idx) => (
          <button
            ...
            style={{
              margin: '0 5px',
              padding: '8px 16px',
              backgroundColor: selectedSchedule.date === sch.date ? '#ccc' : '#007bff',
              color: selectedSchedule.date === sch.date ? '#666' : 'white',
              border: 'none',
              borderRadius: '4px',
              cursor: selectedSchedule.date === sch.date ? 'not-allowed' : 'pointer'
            }}
          >
            {sch.date}
          </button>
